...
tapply(X = iris$Sepal.Length, INDEX = list(iris$Species), FUN = mean)  #tapply cuando es condicional al factor el apply
                #revisar diferencias de apply t apply

#especifica---mean(iris$Sepal.Length[which(iris$Species=="virginica")])
...
freq_sl <- sort(table(iris$Sepal.Length), decreasing = TRUE) #table me bota frecuencias de los valores, el drecreasing es ene l valor de freq
freq_sl[1] #Me muestra el valor con mayor freq
...
boxplot(iris$Sepal.Width)
my_boxplot <- boxplot(iris$Sepal.Width, plot = F)
my_boxplot
