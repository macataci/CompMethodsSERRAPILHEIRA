...
boxplot(cat$growth, col = "lightpink" )
...
ggplot(data = cat, mapping = aes(x = tannin, y = growth)) +
  geom_point() +
  geom_smooth(method = lm) #esto pone la curva de ic creo +
  theme_classic()
...
fln <- fitdist(groundbeef$serving, "lnorm") #loglik mas enano
